...
gcloud run deploy cashsathi-api --image "$repository/cashsathi-api:$tag" --region $Region --allow-unauthenticated --service-account "cashsathi-api@$ProjectId.iam.gserviceaccount.com" --min 0 --max 3 --cpu 1 --memory 1Gi --concurrency 8 --timeout 600 --set-env-vars "APP_ENV=production,GCP_PROJECT_ID=$ProjectId,FIRESTORE_DATABASE_ID=(default),CORS_ALLOWED_ORIGINS=https://bootstrap.invalid,WEB_BASE_URL=https://bootstrap.invalid,LOG_LEVEL=INFO,GEMINI_MODEL=gemini-3.6-flash,MAX_PDF_BYTES=10485760,MAX_PDF_PAGES=25,GEMINI_TIMEOUT_SECONDS=45,GMAIL_KMS_KEY_NAME=projects/$ProjectId/locations/$Region/keyRings/cashsathi/cryptoKeys/gmail-oauth-tokens,SCHEDULER_SERVICE_ACCOUNT_EMAIL=cashsathi-scheduler@$ProjectId.iam.gserviceaccount.com,PLATFORM_ADMIN_UIDS=$escapedAdminUids" --set-secrets "GEMINI_API_KEY=gemini-api-key:latest,GMAIL_OAUTH_CLIENT_ID=gmail-oauth-client-id:latest,GMAIL_OAUTH_CLIENT_SECRET=gmail-oauth-client-secret:latest"
