...
gcloud run deploy cashsathi-api --image "$repository/cashsathi-api:$tag" --region $Region --allow-unauthenticated --service-account "cashsathi-api@$ProjectId.iam.gserviceaccount.com" --min 0 --max 3 --cpu 1 --memory 512Mi --concurrency 80 --timeout 60 --set-env-vars "APP_ENV=production,GCP_PROJECT_ID=$ProjectId,FIRESTORE_DATABASE_ID=(default),CORS_ALLOWED_ORIGINS=https://bootstrap.invalid,LOG_LEVEL=INFO"
