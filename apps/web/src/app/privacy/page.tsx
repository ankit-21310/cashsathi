...
export default function PrivacyPage() {
  ...
  return (
    <AuthenticatedShell><main className="dashboard-main">
      ...
      <section className="foundation-panel"><div className="panel-heading"><div><h2>Founder Recovery Plan</h2><p>Secure one-time checkout; no recurring billing.</p></div></div>{plan ? <div className="policy-preview"><strong>{formatMinorAmount(plan.price_minor, plan.currency)} one-time · {humanize(plan.status)}</strong><span>{plan.invoices_used} of {plan.invoice_limit} confirmed invoices used · {humanize(plan.source)}</span></div> : <div className="centered-state compact">No active plan. Workspace owners can purchase from Billing.</div>}</section>
