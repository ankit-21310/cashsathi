CashSathi — Receivables Operator Preview

GitHub Stars

🌐 Try it Live: https://cashsathi-web.vercel.app/

🎥 Demo Video: Watch on YouTube

⭐ If you find CashSathi useful, please consider giving the repository a star!
...
A team member uploads a PDF invoice. The API validates it, extracts a draft with Gemini,

and discards the PDF bytes after the request.



A human reviews and confirms the customer, amount, currency, invoice number, and due date.



Gemini proposes a structured function call for the next action. The proposal includes

model and prompt metadata for the audit trail.



Backend policy applies cooldown, high-value, dispute, customer, language, payment, and

automation rules. Policy is authoritative over model output.



An allowed reminder is queued for approval or delivered through Gmail. Cloud Scheduler

rechecks invoices that are due for evaluation.



A human records verified payments. Timelines, impact metrics, forecasts, and exports are
...
Full business control, including owner-only consent, privacy, and team decisions.
...
Manages daily operations, approvals, integrations, policies, and non-owner team access.
...
Runs invoice, customer, dispute, payment, and action workflows without administrative control.
...
Reviews business, impact, and finance information; sensitive reminder copy is redacted.
...
No automatic reminder inside the configured cooldown window.
...
Requires human approval at or above the configured threshold.





Manual-only customer
...
Automation stops until an authorized user records a resolution.
...
Prohibited by immutable policy.
...
Only a human-confirmed payment can close an invoice.
...
One bounded retry, then human review.
...
Marked unknown and never automatically resent.
...
or provider responses, and secrets. See the detailed policy baseline,
privacy model, and
...
In a second terminal, seed two isolated demo tenants and start both applications:
...
Use the Vercel Hobby deployment guide for the
no-billing demo path. The cloud setup guide preserves the
future GCP deployment, and the production runbook
covers operations. The
Phase 8 package records the repository-controlled
...
claim launch completion. Genuine customer evidence, screenshots, production verification, event registration, and
external submission remain operator responsibilities.
...
If CashSathi looks useful or interesting, consider giving the project a star on GitHub.

⭐ Star CashSathi on GitHub →
